  ...
  return (
    <aside className="flex h-full w-72 flex-col border-l border-border bg-card">
      <div className="border-b border-border px-4 py-3">
        <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Arrastar para fase</p>
        <h2 className="font-display text-lg font-semibold">Equipa</h2>
        <p className="mt-1 text-[11px] text-muted-foreground">
          Esta semana · {format(thisWeek.start, "MMM d")} – {format(thisWeek.end, "MMM d")}
        </p>
      </div>
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        ...
        {!activeResources.length && (
          <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
            Sem membros activos. Adiciona ou activa alguém na Equipa para começar a alocar.
          </div>
        )}
      </div>
    </aside>
