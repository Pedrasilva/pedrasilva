...
import { allocationCost, dayCount, euros, workingDays, fmt } from "@/lib/projects/gantt-utils";
import {
  useCreateAllocation,
  useUpdateAllocation,
...
import { StageDependencyEditor } from "@/components/projects/StageDependencyEditor";
...
export function GanttChart({ stages, origin, totalDays, dayWidth, resources }: Props) {
  ...
  function commitLinkDrag() {
    ...
    createDep
      .mutateAsync({
        predecessor_id: link.fromStageId,
        successor_id: target,
        type,
        lag_days: 0,
      })
  ...
  function handleDrop(e: React.DragEvent, stage: StageWithProject) {
    ...
    if (movedAlloc) {
      try {
        const parsed = JSON.parse(movedAlloc) as {
          allocationId: string;
        ...
        updateAlloc
          ...
          .then(() => toast.success("Alocação movida"))
          .catch((err) => toast.error((err as Error).message));
      } catch (err) {
        toast.error((err as Error).message);
      }
    ...
    createAlloc
      ...
      .then(() => toast.success("Recurso alocado"))
      .catch((err) => toast.error((err as Error).message));
  }
          ...
          {Array.from({ length: totalDays }).map((_, i) => {
            ...
            return (
              <div
                ...
                } ${isMonStart ? "border-l border-border/60" : "border-l border-border/20"}`}
      ...
      <div className="relative gantt-canvas-bg gantt-week-marker" style={{ minHeight: stages.length * 200 }}>
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ left: stageX, width: stageW, top: 0, height: STAGE_ROW_H }}
                >
                  <div className="absolute left-0 right-0 top-0 h-3 overflow-hidden rounded-t-md bg-muted">
                    <div
                    ...
                    {over && (
                      <div
                        className="absolute right-0 top-0 h-full bg-destructive/80"
                  ...
                  >
                    <div className="flex h-full items-center justify-between px-3 text-foreground">
                        ...
                        <div className="mt-0.5 flex items-center gap-2 text-[11px] opacity-80">
                          ...
                          {over && (
                            <span className="rounded bg-destructive px-1.5 py-px font-medium text-destructive-foreground">
                              ultrapassado em {euros(totalCost - budget)}
                            </span>
                          )}
                        </div>
                      </div>
                      <button
                        onClick={async (e) => {
                          e.stopPropagation();
                          if (!confirm(`Apagar fase "${stage.name}"?`)) return;
                ...
                {stage.allocations.map((a, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ left: aX, width: aW, top, height: ALLOC_ROW_H }}
                    >
        ...
        >
          ...
          {visibleDeps.map((d) => {
            ...
            return (
              <path key={d.id} d={path} fill="none" stroke="hsl(var(--primary))" strokeWidth={1.5} strokeOpacity={0.7} markerEnd="url(#dep-arrow)" />
            );
          })}
          {link && (() => {
            ...
            const fromY = p.top + STAGE_ROW_H / 2;
            return (
              <line x1={fromX} y1={fromY} x2={link.pointerX} y2={link.pointerY} stroke="hsl(var(--primary))" strokeWidth={1.5} strokeDasharray="4 3" />
            );
          })()}
        </svg>
      </div>
