...
type Metric = "Valor" | "Orçamento (h)" | "Nº de projectos";
...
export function PerformanceTable({ projects, stages, resources, loading }: Props) {
  const [metric, setMetric] = useState<Metric>("Orçamento (h)");
    ...
    <section className="rounded-lg border border-border bg-card">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-muted-foreground">Por recurso</span>
        </div>
        <div className="inline-flex items-center gap-1 rounded-md border border-border bg-background p-1">
          {(["Valor", "Orçamento (h)", "Nº de projectos"] as Metric[]).map((m) => (
            <button
              key={m}
              onClick={() => setMetric(m)}
              className={cn(
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/30 text-[10px] uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="px-5 py-2 text-left font-medium">Pessoa</th>
              <th className="px-3 py-2 text-center font-medium"># Activos</th>
              <th className="px-3 py-2 text-right font-medium">Valor</th>
              <th className="px-3 py-2 text-left font-medium">Orçamento</th>
              <th className="px-5 py-2 text-left font-medium">Prazo</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-8 text-center text-xs text-muted-foreground">
                  Sem recursos alocados a projectos.
                </td>
              </tr>
            )}
            {rows.map((r) => (
