...
/**
 * Read-only mirror of a salary snapshot, shared between "Minha ficha" and the
 * collaborator detail page so that both surfaces show the exact same fields.
 *
 * - Renders: snapshot header + KPIs, MonthlyLiquidityCard, ValueChainSummary,
 *   and inner Tabs (Líquido / Bruto / Detalhes / Comparar).
 * - `showSnapshotPicker` controls whether the per-snapshot toggle is shown
 *   (Minha ficha uses it; the collaborator detail page already has outer
 *   per-snapshot Tabs and passes `false`).
 */
export function SnapshotMirrorPanel({
  snapshot,
  collaborator,
  allSnapshots,
  selectedId,
  ...
  // Mirror collaborator household into the snapshot so IRS / dependants
  // come from the canonical source (collaborator), not stale snapshot rows.
  ...
  const { data: fetchedExpenses = [] } = useQuery({
    queryKey: ["mirror-panel-benefit-expenses-12m", collaborator.id],
    enabled: expenses === undefined,
    queryFn: async () => {
      const from = new Date();
  ...
  const benefitExpenses = expenses ?? fetchedExpenses;
  ...
  return (
    <div className="space-y-5">
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {showSnapshotPicker && allSnapshots.length > 1 && onSelect && (
              ...
              {snapshot.is_effective ? (
                ...
                <Badge variant="secondary">{t("hr:myProfile.historical")}</Badge>
              )}
            </div>
          </div>
        </CardHeader>
        <CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Mini label={t("hr:myProfile.mini.grossMonthly")} value={fmtEUR(c.brutoMensal)} />
          <Mini label={t("hr:myProfile.mini.netMonthly")} value={fmtEUR(c.liquido14m)} />
          <Mini label={t("hr:myProfile.mini.netAnnual")} value={fmtEUR(c.liquidoAnual)} />
          <Mini
      ...
      <Tabs defaultValue="liquido">
        <TabsList>
          <TabsTrigger value="liquido">{t("hr:myProfile.tabs.net")}</TabsTrigger>
          <TabsTrigger value="bruto">{t("hr:myProfile.tabs.gross")}</TabsTrigger>
          <TabsTrigger value="details">{t("hr:myProfile.tabs.details")}</TabsTrigger>
          <TabsTrigger value="compare">{t("hr:myProfile.tabs.compare")}</TabsTrigger>
        </TabsList>
...
function DetailsTab({
  ...
  const modoLabel =
    SUBSIDIOS_MODO_OPTIONS.find((o) => o.value === snapshot.subsidios_modo)?.label ??
    snapshot.subsidios_modo;
  const sourceLabel =
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-muted-foreground">{t("hr:myProfile.details.intro")}</p>
      ...
      <Section title={t("hr:myProfile.details.sections.social")}>
        ...
        <Row label={f("ss_colaborador_pct")} value={pct(snapshot.ss_colaborador_pct)} />
        <Row label={f("irs_pct")} value={pct(snapshot.irs_pct)} />
        <Row label={f("irs_calculado_auto")} value={yn(snapshot.irs_calculado_auto)} />
      </Section>
      ...
      <Section title={t("hr:myProfile.details.sections.allowances")}>
        <Row label={f("ajudas_custo_anual")} value={fmtEUR(snapshot.ajudas_custo_anual)} />
