/**
 * Invoices workspace — list of client invoices / credit notes (issued docs).
 *
 * Source of truth: `financial_documents` filtered by direction='issued'.
 * Opens `InvoiceEditorDialog` for create/edit.
 * Mirrors PurchasesWorkspace.
 */
...
export function InvoicesWorkspace() {
  ...
  const clientName = useMemo(() => {
    ...
    for (const c of clientsQ.data ?? []) map.set(c.id, c.name);
    return map;
  }, [clientsQ.data]);
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Kpi
          label={t("finance:invoices.kpi.count")}
          value={String(summary.count)}
        />
        <Kpi
          label={t("finance:invoices.kpi.total")}
          value={fmt(summary.total)}
        />
        <Kpi
          label={t("finance:invoices.kpi.outstanding")}
          value={fmt(summary.outstanding)}
        />
        <Kpi
          label={t("finance:invoices.kpi.overdue")}
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            <CardTitle>{t("finance:invoices.title")}</CardTitle>
            <CardDescription>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-2">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                ...
                placeholder={t("finance:invoices.searchPlaceholder") as string}
                className="pl-8"
              />
            </div>
            <Select
            ...
            >
              <SelectTrigger className="w-full md:w-[220px]">
                <SelectValue placeholder={t("finance:invoices.allClients")} />
              </SelectTrigger>
              <SelectContent className="max-h-[280px]">
                <SelectItem value="all">
                  {t("finance:invoices.allClients")}
                </SelectItem>
                {(clientsQ.data ?? []).map((c) => (
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">
                  {t("finance:invoices.allStatuses")}
                </SelectItem>
                {STATUS_LIST.map((s) => (
                  <SelectItem key={s} value={s}>
                    {t(`finance:invoices.status.${s}`)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[110px]">
                      {t("finance:invoices.col.issueDate")}
                    </TableHead>
                    <TableHead>{t("finance:invoices.col.client")}</TableHead>
                    <TableHead>{t("finance:invoices.col.number")}</TableHead>
                    <TableHead>{t("finance:invoices.col.project")}</TableHead>
                    <TableHead className="w-[120px]">
                      {t("finance:invoices.col.due")}
                    </TableHead>
                    <TableHead className="text-right w-[130px]">
                      {t("finance:invoices.col.total")}
                    </TableHead>
                    <TableHead className="text-right w-[130px]">
                      {t("finance:invoices.col.outstanding")}
                    </TableHead>
                      ...
                      >
                        ...
                        <TableCell className="font-medium">
                          {d.counterparty_client_id
                            ? clientName.get(d.counterparty_client_id) ??
