// Extracted from the previous monolithic /finance route so the new
// layout can mount each tab as its own child route. Behavior unchanged.
...
export const FINANCE_YEAR = 2026;
...
export function buildCashFlow(
  ...
  for (const p of periods) {
    ...
    const opening: number =
      seedOpening > 0 || runningOpening == null
        ? seedOpening || (runningOpening ?? 0)
        : runningOpening;
...
// ---------------------------------------------------------------------------
// Overview (legacy KPI block, kept for compatibility — wrapped by FinanceOverview)
// ---------------------------------------------------------------------------

export function OverviewKpiBlock({ vatMode }: { vatMode: VatMode }) {
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          tone={
            summary.projectedClosing >= summary.currentBank ? "positive" : "negative"
          }
        />
        <KpiCard
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("finance:bank.col.latest")}</TableHead>
...
// ---------------------------------------------------------------------------
// Shared query helpers reused by Income / Expenses sections
// ---------------------------------------------------------------------------
...
export function IncomeSection({ vatMode }: { vatMode: VatMode }) {
  const { t } = useTranslation(["finance", "common"]);
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((r) => {
      ...
      if (statusFilter !== "all" && r.invoice_status !== statusFilter) return false;
      return true;
    });
      ...
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <MonthFilter value={monthFilter} onChange={setMonthFilter} periods={periodOptions} />
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[180px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("finance:filter.allStatuses")}</SelectItem>
                <SelectItem value="planned">{t("finance:invoiceStatus.planned")}</SelectItem>
                <SelectItem value="issued">{t("finance:invoiceStatus.issued")}</SelectItem>
                <SelectItem value="paid">{t("finance:invoiceStatus.paid")}</SelectItem>
                <SelectItem value="overdue">{t("finance:invoiceStatus.overdue")}</SelectItem>
        ...
        <div className="flex flex-wrap gap-6 text-xs">
          <SummaryStat label={t("finance:income.summary.total")} value={fmtEUR(totals.total)} />
          <SummaryStat label={t("finance:income.summary.paid")} value={fmtEUR(totals.paid)} tone="text-emerald-700" />
          <SummaryStat label={t("finance:income.summary.outstanding")} value={fmtEUR(totals.outstanding)} tone="text-amber-700" />
        </div>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("finance:income.col.amount")}</TableHead>
            ...
            {rows.map((r) => {
              ...
              return (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-sm tabular-nums">{r.invoice_number ?? "—"}</TableCell>
                  <TableCell className="text-sm tabular-nums">{fmtDate(r.expected_payment_date)}</TableCell>
                  <TableCell className="text-sm tabular-nums">{fmtDate(r.paid_date)}</TableCell>
                  <TableCell className="text-right tabular-nums">{fmtEUR2(amt)}</TableCell>
                  <TableCell>
          ...
          {rows.length > 0 && (
            <TableFooter>
              <TableRow>
                <TableCell colSpan={6}>{t("finance:income.summary.total")}</TableCell>
                <TableCell className="text-right tabular-nums font-semibold">{fmtEUR2(totals.total)}</TableCell>
                <TableCell />
              </TableRow>
            </TableFooter>
          )}
...
// ---------------------------------------------------------------------------
// Expenses (operational + project costs combined view; filter via kind)
// ---------------------------------------------------------------------------
...
export function ExpensesSection({
  ...
  vatMode: VatMode;
  kind: "all" | "operational" | "projectCosts";
}) {
  const { t } = useTranslation(["finance", "common"]);
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((r) => {
      if (kind === "projectCosts" && r.expense_type !== "materials") return false;
      if (kind === "operational" && r.expense_type === "materials") return false;
      if (monthFilter !== "all" && r.period_id !== monthFilter) return false;
  ...
  const titleKey =
    kind === "projectCosts"
      ? "finance:projectCosts.title"
      : kind === "operational"
        ? "finance:expenses.title"
        : "finance:expensesAll.title";
  const subtitleKey =
    kind === "projectCosts"
      ? "finance:projectCosts.subtitle"
      : kind === "operational"
        ? "finance:expenses.subtitle"
        : "finance:expensesAll.subtitle";
  const emptyKey =
    kind === "projectCosts"
      ? "finance:projectCosts.empty"
      : kind === "operational"
        ? "finance:expenses.empty"
        : "finance:expensesAll.empty";
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">{t("finance:filter.allStatuses")}</SelectItem>
                <SelectItem value="projected">{t("finance:expenseStatus.projected")}</SelectItem>
                <SelectItem value="confirmed">{t("finance:expenseStatus.confirmed")}</SelectItem>
                <SelectItem value="paid">{t("finance:expenseStatus.paid")}</SelectItem>
                <SelectItem value="overdue">{t("finance:expenseStatus.overdue")}</SelectItem>
                <SelectItem value="cancelled">{t("finance:expenseStatus.cancelled")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        <div className="flex flex-wrap gap-6 text-xs">
          <SummaryStat label={t("finance:expenses.summary.total")} value={fmtEUR(totals.total)} />
          <SummaryStat label={t("finance:expenses.summary.paid")} value={fmtEUR(totals.paid)} tone="text-emerald-700" />
          <SummaryStat label={t("finance:expenses.summary.outstanding")} value={fmtEUR(totals.outstanding)} tone="text-rose-700" />
        </div>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("finance:expenses.col.amount")}</TableHead>
          ...
          <TableBody>
            ...
            {rows.map((r) => {
              ...
              return (
                <TableRow key={r.id}>
                  <TableCell className="text-sm">{period?.month_name ?? DASH}</TableCell>
                  <TableCell className="text-sm">{supplier ?? DASH}</TableCell>
                  <TableCell className="text-sm">{category ?? DASH}</TableCell>
                  <TableCell className="text-sm max-w-[280px] truncate">{r.description ?? DASH}</TableCell>
                  <TableCell className="text-sm tabular-nums">{fmtDate(r.due_date)}</TableCell>
                  <TableCell className="text-sm tabular-nums">{fmtDate(r.paid_date)}</TableCell>
                  <TableCell className="text-right tabular-nums">{fmtEUR2(amt)}</TableCell>
                  <TableCell>
          ...
          {rows.length > 0 && (
            <TableFooter>
              <TableRow>
                <TableCell colSpan={6}>{t("finance:expenses.summary.total")}</TableCell>
                <TableCell className="text-right tabular-nums font-semibold">{fmtEUR2(totals.total)}</TableCell>
                <TableCell />
                {isAdmin && <TableCell />}
              </TableRow>
...
export function DebtsSection() {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <CardTitle className="font-display text-xl">{t("finance:debts.title")}</CardTitle>
              <p className="text-xs text-muted-foreground mt-1">
                {t("finance:debts.subtitle", { count: debts.length })}
              </p>
            </div>
            <div className="flex flex-wrap gap-6 text-xs">
              <SummaryStat label={t("finance:debts.summary.original")} value={fmtEUR(totalOriginal)} />
              <SummaryStat
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">{t("finance:debts.col.original")}</TableHead>
                <TableHead className="text-right">{t("finance:debts.col.outstanding")}</TableHead>
                ...
                <TableHead className="text-right">{t("finance:debts.col.scheduled")}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {debts.length === 0 && (
              ...
              {debts.map((d) => {
                ...
                return (
                  <TableRow key={d.id}>
                    <TableCell className="text-sm font-medium">{d.creditor_name}</TableCell>
                    <TableCell className="text-sm max-w-[280px] truncate">{d.description ?? DASH}</TableCell>
                    <TableCell className="text-right tabular-nums">{fmtEUR2(Number(d.original_amount))}</TableCell>
                    <TableCell className="text-right tabular-nums">{fmtEUR2(Number(d.outstanding_amount))}</TableCell>
                    <TableCell>
                      <DebtStatusBadge status={d.status} />
                    </TableCell>
                    <TableCell className="text-sm tabular-nums">{fmtDate(d.start_date)}</TableCell>
                    <TableCell className="text-sm tabular-nums">{fmtDate(d.end_date)}</TableCell>
                    <TableCell className="text-right tabular-nums">
                      {scheduled > 0 ? t("finance:debts.scheduledCount", { count: scheduled }) : DASH}
                    </TableCell>
                  </TableRow>
                );
              })}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-display text-xl">{t("finance:debts.payments.title")}</CardTitle>
          <p className="text-xs text-muted-foreground mt-1">
            {t("finance:debts.payments.subtitle", { count: paymentsQ.data?.length ?? 0 })}
          </p>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">{t("finance:debts.payments.col.planned")}</TableHead>
                  <TableHead className="text-right">{t("finance:debts.payments.col.actual")}</TableHead>
              ...
              <TableBody>
                {(paymentsQ.data ?? []).map((p) => {
                  ...
                  const period = p.period_id ? periodsQ.data?.get(p.period_id) : null;
                  return (
                    <TableRow key={p.id}>
                      <TableCell className="text-sm">{debt?.creditor_name ?? DASH}</TableCell>
                      <TableCell className="text-sm">{period ?? DASH}</TableCell>
                      <TableCell className="text-sm tabular-nums">{fmtDate(p.due_date)}</TableCell>
                      <TableCell className="text-sm tabular-nums">{fmtDate(p.paid_date)}</TableCell>
                      <TableCell className="text-right tabular-nums">{fmtEUR2(Number(p.planned_amount))}</TableCell>
                      <TableCell className="text-right tabular-nums">
                        {p.actual_amount != null ? fmtEUR2(Number(p.actual_amount)) : DASH}
                      </TableCell>
                      <TableCell>
                        <Badge variant="secondary" className="font-normal">
                          {t(`finance:debtPaymentStatus.${p.status}`, { defaultValue: p.status })}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  );
...
export function ImportLogsSection() {
  const { t } = useTranslation(["finance", "common"]);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-display text-xl">{t("finance:importLogs.title")}</CardTitle>
        <p className="text-xs text-muted-foreground mt-1">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("finance:importLogs.col.rowsExpenses")}</TableHead>
              <TableHead className="text-right">{t("finance:importLogs.col.rowsIncome")}</TableHead>
              <TableHead className="text-right">{t("finance:importLogs.col.rowsSuppliers")}</TableHead>
              <TableHead className="text-right">{t("finance:importLogs.col.rowsClients")}</TableHead>
              <TableHead className="text-right">{t("finance:importLogs.col.rowsDebts")}</TableHead>
              <TableHead className="text-right">{t("finance:importLogs.col.rowsBankAccounts")}</TableHead>
          ...
          <TableBody>
            ...
            {logs.map((l) => (
              <TableRow key={l.id}>
                <TableCell className="text-sm tabular-nums whitespace-nowrap">{fmtDateTime(l.imported_at)}</TableCell>
                <TableCell className="text-sm max-w-[220px] truncate">{l.file_name}</TableCell>
                <TableCell className="text-sm">
                  <Badge variant="secondary" className="font-normal">{l.import_type}</Badge>
                </TableCell>
                <TableCell className="text-sm tabular-nums">{fmtBytes(l.source_file_size_bytes)}</TableCell>
                <TableCell className="text-xs font-mono text-muted-foreground">
                  {l.file_checksum ? (
                    <code className="select-all cursor-text rounded bg-muted px-1.5 py-0.5" title={l.file_checksum}>
                ...
                <TableCell className="text-right tabular-nums">{l.rows_expenses}</TableCell>
                <TableCell className="text-right tabular-nums">{l.rows_income}</TableCell>
                <TableCell className="text-right tabular-nums">{l.rows_suppliers}</TableCell>
                <TableCell className="text-right tabular-nums">{l.rows_clients}</TableCell>
                <TableCell className="text-right tabular-nums">{l.rows_debts}</TableCell>
                <TableCell className="text-right tabular-nums">{l.rows_bank_accounts}</TableCell>
                <TableCell className="text-sm max-w-[200px] truncate text-muted-foreground">{l.notes ?? DASH}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
...
export function PlaceholderSection({ titleKey }: { titleKey?: string }) {
  const { t } = useTranslation(["finance"]);
  return (
    <Card>
      <CardContent className="py-16 text-center text-sm text-muted-foreground">
        <div className="font-display text-lg text-foreground">
          {titleKey ? t(titleKey) : t("finance:tabPlaceholder.title")}
        </div>
