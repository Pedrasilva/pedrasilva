...
export function useProjectInsights(projectId: string) {
  return useQuery({
    queryKey: ["pm_project-insights", projectId],
    enabled: !!projectId,
    queryFn: async (): Promise<ProjectInsights> => {
      const { data: stages, error: sErr } = await supabase
        .from("pm_stages")
        .select(
          "id, budget, allocations:pm_allocations(id, hours_per_day, start_date, end_date, resource:pm_resources(id, name, color, sale_rate, cost_rate), tasks:pm_tasks(id))",
        )
      ...
      type AllocLite = {
        ...
        start_date: string;
        end_date: string;
        resource: { id: string; name: string; color: string; sale_rate: number; cost_rate: number };
        tasks: { id: string }[];
      };
      ...
      for (const e of entries) {
        ...
        loggedHours += hours;
        const sale = hours * Number(alloc.resource.sale_rate);
        const cost = hours * Number(alloc.resource.cost_rate);
        earnedValue += sale;
        totalCost += cost;
      ...
      const monthly = Array.from(monthMap.values()).sort((a, b) => (a.monthKey < b.monthKey ? -1 : 1));
      ...
      const total: FinancialsRow = { ...services };
