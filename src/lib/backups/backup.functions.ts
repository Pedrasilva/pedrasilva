...
// All business tables we want preserved. Order is not significant — we export each
// independently. Keep this list in sync with new tables you want included in backups.
...
const TRIGGERS = ["daily", "weekly", "manual"] as const;
...
async function ensureSubfolder(parentId: string, name: string): Promise<string> {
  // Try to find an existing folder with this name under the parent
...
async function dumpAllTables(): Promise<{
  ...
  for (const t of BACKUP_TABLES) {
    try {
      // Page through to avoid PostgREST cap
      const pageSize = 1000;
      let from = 0;
      const all: unknown[] = [];
...
async function performBackup(trigger: Trigger, triggeredBy: string | null) {
...
// Public webhook entry, called by pg_cron. Authenticated by shared secret.
export async function runScheduledBackup(trigger: Trigger) {
  return performBackup(trigger, null);
}
