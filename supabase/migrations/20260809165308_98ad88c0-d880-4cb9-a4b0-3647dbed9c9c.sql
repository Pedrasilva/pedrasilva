...
BEGIN
  ...
  UPDATE public.financial_documents
     SET paid_amount = v_paid,
         status      = v_status,
         -- A bank-matched but only PARTIALLY paid document still owes money and
         -- must stay in the receivables/payables pipeline.
         payment_status = CASE
           WHEN v_bank_matched AND v_paid >= v_total THEN 'reconciled'
