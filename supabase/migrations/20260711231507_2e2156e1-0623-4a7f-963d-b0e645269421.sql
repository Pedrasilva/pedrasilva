...
BEGIN
  ...
  EXECUTE format(
    'INSERT INTO financial_documents (%s) SELECT %s FROM jsonb_populate_recordset(NULL::financial_documents, $1)',
    cols, cols
  ...
  EXECUTE format(
    'INSERT INTO financial_document_lines (%s) SELECT %s FROM jsonb_populate_recordset(NULL::financial_document_lines, $1)',
    cols, cols
