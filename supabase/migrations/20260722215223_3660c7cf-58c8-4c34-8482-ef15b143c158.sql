...
BEGIN
  ...
  WITH ins AS (
    ...
    SELECT
      ...
      'custom_text'::public.psa_block_type,
      COALESCE(tb.block_title, pb.title, 'Bloco'),
      'library'::public.psa_block_source_type,
      jsonb_build_object('template_id', _template_id, 'template_block_id', tb.id),
      jsonb_build_object('html', COALESCE(pb.default_content, '')),
      'proposal_only'::public.psa_contract_relevance
...
GRANT EXECUTE ON FUNCTION public.psa_import_template_blocks(uuid, uuid) TO authenticated;
